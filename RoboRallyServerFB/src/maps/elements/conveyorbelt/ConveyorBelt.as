package maps.elements.conveyorbelt
{
	...
	{
		[Embed(source="assets/conveyorbelt.png")]
		private var conveyorBeltClass1:Class;
		[Embed(source="assets/conveyorbeltLeft.png")]
		private var conveyorBeltClassLeft:Class;
		[Embed(source="assets/conveyorbeltUp.png")]
		private var conveyorBeltClassUp:Class;
		[Embed(source="assets/conveyorbeltDown.png")]
		private var conveyorBeltClassDown:Class;
		...
		public function ConveyorBelt(selectable:Boolean, deletable:Boolean)
		{
		...
		override public function doAction(robot:PlayerBoardElement, aElementFinished:Function, finishedFunction:Function, length:uint, registerIndex:int):void
		{
			if (robot != null)
			{
				...
				{
					case 0: 
						robot.playerAction.move(new Point(1, 0), 1, "conveyorBelt", aElementFinished, length, finishedFunction);
						break;
					case 1: 
						robot.playerAction.move(new Point(0, 1), 1, "conveyorBelt", aElementFinished, length, finishedFunction);
						break;
					case 2: 
						robot.playerAction.move(new Point(-1, 0), 1, "conveyorBelt", aElementFinished, length, finishedFunction);
						break;
		...
		private function changeRighNodes(node:Node, switchtoid:int):void
		{
			...
			while (true)
			{
				...
				{
					if ((node.boardElement as ConveyorBelt).id == _id)
					{
						(node.boardElement as ConveyorBelt).id = switchtoid;
					}
					else
						return;
				}
